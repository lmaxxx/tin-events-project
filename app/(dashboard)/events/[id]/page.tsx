  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <Card className={"p-0 pb-4"}>
        <div className="aspect-2/1 overflow-hidden bg-neutral-100 dark:bg-neutral-800 relative">
          {event.imageUrl ? (
            <img
              src={event.imageUrl}
              alt={event.title}
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {isPastEvent && <Badge variant="secondary">{tCommon('status.eventEnded')}</Badge>}
                {!isPastEvent && isFull && <Badge variant="destructive">{tCommon('status.full')}</Badge>}
              </div>
            </div>
            {canManage && (
          ...
          <div className="grid md:grid-cols-2 gap-4 pt-4 border-t">
            ...
            <div className="space-y-3">
              ...
              {user ? (
                <div className="pt-4">
                  {isRegistered ? (
                    <Button
                      ...
                      disabled={unregisterMutation.isPending || isPastEvent}
                    >
                      {unregisterMutation.isPending ? t('unregistering') : isPastEvent ? tCommon('status.eventEnded') : t('unregister')}
                    </Button>
                  ) : (
                    <Button
                      className="w-full"
                      onClick={() => registerMutation.mutate()}
                      disabled={registerMutation.isPending || isFull || isPastEvent}
                    >
                      {registerMutation.isPending
                        ? t('registering')
                        : isPastEvent
                        ? tCommon('status.eventEnded')
                        : isFull
                        ? t('eventFull')
                        : t('register')}
                    </Button>
                ...
                <div className="pt-4">
                  <Button className="w-full" disabled={isPastEvent} asChild={!isPastEvent}>
                    {isPastEvent ? (
                      tCommon('status.eventEnded')
                    ) : (
